		...
		if ($this->_options['display_buttons']) {
			...
			if ($this->_options['display_reset']) {
				...
				$reset_button_options = array(
					...
					'data-xm_link' => URL::site(Request::current()->uri()), // this will return the current uri
				);
			...
			if ($this->_options['display_cancel']) {
				$cancel_button_options = array(
					'class' => 'js_xm_button_link',
					'data-xm_link' => URL::site(Route::get($target_route)->uri(array('model' => $this->model_name(), 'action' => 'cancel'))),
				);
	...
	public function send_file($response, $column_name) {
		if ( ! empty($this->$column_name)) {
			...
			if ( ! file_exists($file_path)) {
				throw new XM_Exception_File('The file that was attempted to be sent to the browser does not exist: :file:', array(':file:' => $file_path), XM_Exception_File::FILE_DOES_NOT_EXIST);
			}
